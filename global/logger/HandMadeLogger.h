...
public:
    Logger(LoggerSettings _loggerSettings, const LogMessageSettings _defaultLogMessageSettings) : loggerSettings(
            std::move(
                    _loggerSettings)), defaultLogMessageSettings(_defaultLogMessageSettings) {};

    void logHttpRequest(const httplib::Request &req, const httplib::Response &res) const {
        ...
        std::string logString = "Request: " + req.method + " " + req.path + " " + req.version + "\n"
                + "Response: " + res.version + " " + to_string(res.status) + " " + res.body + "\n";
        ...
        log(logString, computedLogMessageSettings);
    }
